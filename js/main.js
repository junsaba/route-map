...
function draw() {
    ...
    ctx.fillStyle = "#000";
    for (s1 in stations) {
        ctx.fillText(stations[s1][0], stations[s1][1], stations[s1][2]);
    };
};
...
onload = () => {
    ...
    document.onmouseup = () => mouse_drag = false;
    canvas.onmousemove = e => {
        ...
        document.getElementsByClassName("coord")[0].innerText = `${map_x * -1 + e.offsetX - document.body.clientWidth / 2}, ${map_y * -1 + e.offsetY - document.body.clientHeight / 2}`;
    };
};
